...
Either way, restart PyMOL (or `run` the file) and you have 7 new commands:
`rfd3_movie`, `style_fixed`, `apply_camera`, `draw_connectors`,
...
`reverse=1` is worth using by default — on real trajectories, raw file state 1
is the folded structure and state N is noise, so `reverse=1` makes frame 1 show
noise and the last frame show the folded design. If your trajectory renders
backwards, drop it.
